...
struct _RuntimeStack *get_current_frame() {
  ...
  if (rec) return rec->current_frame;
...
void _npop_frame(unsigned int n) {
  ...
  for (i = n; i <= n; i--) {
    ...
    if (current_frame->cleanup != NULL) current_frame->cleanup(current_frame);
...
struct _RuntimeStack *_frame_until(int tag, int do_pop) {
  ...
  while (current_frame != NULL && current_frame->tag != tag) {
    if (do_pop && current_frame->cleanup != NULL) current_frame->cleanup(current_frame);
    current_frame = current_frame->next;
  }
  if (do_pop) set_current_frame(current_frame);
