...
static void *grow_region(void *vr, unsigned int s, bufsize *bget_incr) {
  ...
  if (next_size < s) next_size = s + default_region_page_size;
...
static void get_first_region_page(struct _RegionHandle *r, unsigned int s, int init_bget) {
  struct _RegionPage *p;
  unsigned int page_size = default_region_page_size < s ? s : default_region_page_size;
  p = (struct _RegionPage *)_bounded_GC_calloc(sizeof(struct _RegionPage) + page_size, 1, __FILE__,
                                               __LINE__);
  if (p == NULL) _throw_badalloc();
#ifdef CYC_REGION_PROFILE
  p->total_bytes = GC_size(p);
...
static void *region_vmalloc_impl(struct _RegionHandle *r, unsigned int s) {
  ...
  if (!allocd_page) _throw_badalloc();
...
// allocate s bytes within region r
static void *region_calloc_impl(struct _RegionHandle *r, _AliasQualHandle_t aq, unsigned int n,
                                unsigned int t) {
  unsigned int s = n * t;
  char *result;
  if (r == CYC_CORE_HEAP_REGION) {
...
static void free_region_impl(struct _RegionHandle *r) {
  ...
  while (pl) {
    int i;
    for (i = 0; i < pl->count; i++) drop_refptr_base(pl->pointers[i]);
    {
...
static void reap_rufree_impl(struct _RegionHandle *r, unsigned char *ptr) {
  if (ptr == NULL || r == NULL || r->key == NULL) return;
#ifdef CYC_REGION_PROFILE
  ...
  if (alloc_log != NULL) {
    fprintf(alloc_log, "%u @\tunique\talloc\t-%d\t%d\t%d\t%d\t%x\n", clock(), sz,
            ...
            region_get_total_bytes(CYC_CORE_HEAP_REGION), (unsigned int)ptr);
  }
#endif
...
static void *reap_malloc_impl(struct _RegionHandle *r, _AliasQualHandle_t aq, unsigned int s) {
  ...
  if (r == CYC_CORE_HEAP_REGION) {
    ...
    if (aq == CYC_CORE_REFCNT_AQUAL) {
      ...
      if (!result) _throw_bad_reapalloc();
      *((int *)result) = 1;
#ifdef CYC_REGION_PROFILE
...
// allocate s bytes within region r
static void *reap_calloc_impl(struct _RegionHandle *r, _AliasQualHandle_t aq, unsigned int n,
                              unsigned int t) {
  ...
  if (cnt != NULL) {
    *cnt = *cnt - 1;
    if (*cnt == 0) {  // no more references //add something to bget to update page usage stats
      ...
      if (alloc_log != NULL) {
        fprintf(
            alloc_log, "%u @\trefcnt\talloc\t-%d\t%d\t%d\t%d\t%x\n", clock(), sz,
            region_get_heap_size(CYC_CORE_HEAP_REGION), region_get_free_bytes(CYC_CORE_HEAP_REGION),
            region_get_total_bytes(CYC_CORE_HEAP_REGION), (unsigned int)(ptr.base - sizeof(int)));
      }
#endif
...
void *_region_calloc(struct _RegionHandle *r, _AliasQualHandle_t aq, unsigned int n,
                     unsigned int t) {
  if (r != NULL) return r->fcns->_calloc(r, aq, n, t);
  return reap_calloc_impl(r, aq, n, t);
}
void *_aqual_malloc(_AliasQualHandle_t aq, unsigned int s) { return reap_aqual_malloc_impl(aq, s); }
    ...
    if (*cnt == 0) {  // no more references
      ...
      if (alloc_log != NULL) {
        fprintf(alloc_log, "%u @\trefcnt\talloc\t-%d\t%d\t%d\t%d\t%x\n", clock(), sz,
                region_get_heap_size(CYC_CORE_HEAP_REGION),
                region_get_free_bytes(CYC_CORE_HEAP_REGION),
                region_get_total_bytes(CYC_CORE_HEAP_REGION), (unsigned int)(ptr - sizeof(int)));
      }
...
void _fini_regions() {
  ...
  if (alloc_log != NULL) fclose(alloc_log);
#endif
}
...
int Cyc_set_default_region_page_size(size_t s) {
  if (s < CYC_MIN_REGION_PAGE_SIZE) return 0;
...
struct Cyc_Core_NewDynamicRegion Cyc_Core__new_rckey(unsigned int dis_reaps, const char *file,
  ...
  struct Cyc_Core_NewDynamicRegion res;
  int *krc =
      _bounded_GC_malloc(sizeof(int) + sizeof(struct Cyc_Core_DynamicRegion), __FILE__, __LINE__);
  // errprintf("creating rckey.  Initial address is %x\n",krc);fflush(stderr);
  if (!krc) _throw_badalloc();
  *krc = 1;
...
static void reclaim_finalizer(GC_PTR obj, GC_PTR client_data) {
  if (alloc_log != NULL) fprintf(alloc_log, "%u @ @ reclaim \t%x\n", clock(), (unsigned int)obj);
}
...
struct _RegionHandle _profile_new_region(unsigned int i, const char *rgn_name, const char *file,
                                         const char *func, int lineno) {
  int len;
  char *buf;
  static int cnt = 0;
...
#ifdef CYC_REGION_PROFILE
  if (alloc_log != NULL) fprintf(alloc_log, "%u @\t@\tmark\t%s\n", clock(), s);
#endif
}
