...
#ifndef NODEBUG
#include <stdarg.h>
__attribute__((format(printf, 1, 2))) static void gc_debug(const char *format, ...) {
  fprintf(stderr, "GC: ");
  va_list args;
...
static void *gc_get_memory(void) {
  ...
  for (size_t i = 0; i < GC_REGION_SIZE * GC_NUM_REGIONS; i += increment) {
    void *addr;
    if ((addr = VirtualAlloc(GC_MEMORY + i, increment, MEM_RESERVE, PAGE_READWRITE)) !=
        GC_MEMORY + i)
      return NULL;
  }
  return GC_MEMORY;
...
static void gc_free_memory(void *ptr, size_t size) { VirtualFree(ptr, 0, MEM_RELEASE); }
static void gc_zero_memory(void *ptr, size_t size) { memset(ptr, 0, size + GC_PAGESIZE); }
static int gc_protect_memory(void *ptr, size_t size) {
  void *ptr1 = (void *)(((uintptr_t)ptr / GC_PAGESIZE) * GC_PAGESIZE);
  void *result = VirtualAlloc(ptr1, size + (ptr - ptr1), MEM_COMMIT, PAGE_READWRITE);
  return (ptr1 != result);
}
struct _TEB {
  NT_TIB NtTib;
};
static void *gc_get_stackbottom(void) { return NtCurrentTeb()->NtTib.StackBase; }
#else /* __MINGW32__ */
...
#endif /* __APPLE__ */
  void *ptr =
      mmap(GC_MEMORY, GC_REGION_SIZE * GC_NUM_REGIONS, PROT_READ | PROT_WRITE, flags, -1, 0);
...
static void gc_free_memory(void *ptr, size_t size) { munmap(ptr, size); }
static int gc_protect_memory(void *ptr, size_t size) {
  void *ptr1 = (void *)(((uintptr_t)ptr / GC_PAGESIZE) * GC_PAGESIZE);
...
extern bool GC_init(void) {
  ...
  if (sizeof(void *) != sizeof(uint64_t) || sizeof(double) != sizeof(uint64_t)) {
    errno = ENOEXEC;
    return false;
  }
 ...
 * GC enable/disable.
 */
extern void GC_disable(void) { gc_enabled = false; }
extern void GC_enable(void) { gc_enabled = true; }
 ...
 */
extern void GC_error(gc_error_func_t func) { gc_error_func = func; }
