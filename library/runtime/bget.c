...
#define SizeQuant                       \
  16 /* Buffer allocation size quantum: \
...
struct qlinks {
  ...
  __attribute__((aligned)) struct bfhead *blink; /* Backward link */
};
...
struct bhead {
  ...
  __attribute__((aligned)) bufsize bsize; /* Buffer size: positive if free,
...
struct bget_region {
  ...
  __attribute__((aligned)) bufsize pool_len; /* 0: no bpool calls have been made
...
void brel(bget_rgn_key_t regkey, void *buf) {
...
#endif            /* BufStats */
#ifdef FreeWipe
    V memset((char *)buf, 0x55, (MemSize)(bdh->tsize - sizeof(struct bdhead)));
#endif                   /* FreeWipe */
  ...
  *totalloc += b->bh.bsize;
  ...
  if (relfcn != NULL && ((bufsize)b->bh.bsize) == (*pool_len - sizeof(struct bhead))) {
    assert(b->bh.prevfree == 0);
...
void bectl(bget_rgn_key_t regkey,
           ...
           void(*release) _((bget_rgn_key_t regkey, void *buf)), bufsize pool_incr) {
...
void bpool(bget_rgn_key_t regkey, void *buf, bufsize len) {
...
void bstats(bget_rgn_key_t regkey, bufsize *curalloc, bufsize *totfree, bufsize *maxfree,
            long *nget, long *nrel) {
  bget_region_t reg = regkey->reg;
...
void bstatse(bget_rgn_key_t regkey, bufsize *pool_incr, long *npool, long *npget, long *nprel,
             long *ndget, long *ndrel) {
  bget_region_t reg = regkey->reg;
...
void bufdump(bget_rgn_key_t regkey, void *buf) {
  struct bfhead *b;
  unsigned char *bdump;
  bufsize bdlen;
...
void bpoold(bget_rgn_key_t regkey, void *buf, int dumpalloc, int dumpfree) {
  struct bfhead *b = BFH(buf);
...
int bpoolv(bget_rgn_key_t regkey, void *buf) {
  struct bfhead *b = BFH(buf);
...
bget_rgn_key_t bget_init_region(void *buf, bufsize len) {
  ...
  regkey->priv = 0;
  bget_region_t reg = regkey->reg;
  char *ptr = (((char *)buf) + (sizeof(struct bget_region_key) + sizeof(struct bget_region)));
  if ((size_t)ptr % _Alignof(struct bhead) != 0) {
    ptr += _Alignof(struct bhead) - (size_t)ptr % _Alignof(struct bhead);
  }
  assert((size_t)ptr % _Alignof(struct bhead) == 0);
  V memset(reg, 0, sizeof(struct bget_region));
  reg->id = GRAB_NEXT_ID;
  reg->freelist.ql.flink = reg->freelist.ql.blink = &reg->freelist;
  bpool(regkey, ptr, len - (sizeof(struct bget_region_key) + sizeof(struct bget_region)));
  return regkey;
}
