...
static _Thread_local const char *_exn_filename = "?";
static _Thread_local int _exn_lineno = 0;  // MWH: should be unsigned? Must match core.h
static _Thread_local void *_exn_thrown = NULL;
#endif
...
const char *Cyc_Core_get_exn_name(void *x) { return *(char **)x; }

void Cyc_Core_set_uncaught_exn_fun(int f()) { uncaught_fun = f; }
...
void _set_top_handler() {
  ...
  if (status) {
    ...
    errquit("Uncaught exception %s thrown from around %s:%d\n", exn_name, exn_filename, exn_lineno);
  }
  _push_handler(&top_handler);
}

void *_throw_fn(void *e, const char *filename, unsigned lineno) {
  // If filename starts with "bazel-out/.../bin/", then
  // strip it off.
  if (strncmp(filename, "bazel-out/", 10) == 0) {
    // Strip off 2 directories and the trailing slash
    for (int i = 0; i < 2; i++) {
