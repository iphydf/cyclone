...
char *_zero_arr_plus_char_fn(char *orig_x, unsigned int orig_sz, int orig_i, const char *filename,
                             unsigned lineno) {
  DO_PLUS_FN(char);
}
void *_zero_arr_plus_other_fn(unsigned t, void *orig_x, unsigned orig_sz, int orig_i,
  ...
  switch (t) {
    ...
    case 8:
      DO_PLUS_FN(int64_t);
    default:
      if (t == sizeof(long double)) DO_PLUS_FN(long double);
...
unsigned _get_zero_arr_size_char(const char *orig_x, unsigned orig_offset) { DO_GET_SIZE(char) }
unsigned _get_zero_arr_size_other(unsigned t, const void *orig_x, unsigned orig_offset) {
  switch (t) {
    ...
    case 8:
      DO_GET_SIZE(int64_t);
    default:
      if (t == sizeof(long double)) DO_GET_SIZE(long double);
      _throw_null_fn("runtime_zeroterm.c", __LINE__);
...
void *_zero_arr_inplace_plus_other_fn(unsigned t, void **x, int orig_i, const char *filename,
                                      unsigned lineno) {
...
char *_zero_arr_inplace_plus_post_char_fn(char **x, int orig_i, const char *filename,
                                          unsigned lineno) {
  char *_zap_res = *x;
  *x = _zero_arr_plus_char_fn(_zap_res, 1, orig_i, filename, lineno);
  return _zap_res;
}
void *_zero_arr_inplace_plus_post_voidstar_fn(unsigned t, void **x, int orig_i,
                                              const char *filename, unsigned lineno) {
  void **_zap_res = *x;
...
struct _fat_ptr _fat_ptr_decrease_size(struct _fat_ptr x, size_t sz, size_t numelts) {
