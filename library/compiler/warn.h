...
namespace Warn {
  ...
  void warn(Position::seg_t, string_t fmt, ... inject parg_t) __attribute__((format(printf, 2, 3)));
  void flush_warnings();

  void verr (Position::seg_t, string_t fmt, parg_t ?);

  void err(Position::seg_t, string_t fmt, ... inject parg_t) __attribute__((format(printf, 2, 3)));

  `a vimpos (string_t fmt, parg_t ?ap)
    __attribute__ ((noreturn));

  `a impos(string_t fmt, ... inject parg_t) __attribute__((format(printf, 1, 2), noreturn));
