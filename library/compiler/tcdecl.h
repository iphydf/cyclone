...
#include "absyn.h"
#include "cyclone/library/std/cycboot.h"
...
namespace Tcdecl {
  ...
  struct Aggrdecl *merge_aggrdecl(aggrdecl_t, aggrdecl_t, seg_t, string_t *msg);

  struct Datatypedecl *merge_datatypedecl(datatypedecl_t, datatypedecl_t, seg_t, string_t *msg);
  struct Enumdecl *merge_enumdecl(enumdecl_t, enumdecl_t, seg_t, string_t *msg);
  struct Vardecl *merge_vardecl(vardecl_t, vardecl_t, seg_t, string_t *msg);
  struct Typedefdecl *merge_typedefdecl(typedefdecl_t, typedefdecl_t, seg_t, string_t *msg);

  datatype Binding *merge_binding(binding_t, binding_t, seg_t, string_t *msg);

  struct Xdatatypefielddecl *merge_xdatatypefielddecl(xdatatypefielddecl_t, xdatatypefielddecl_t,
                                                      seg_t, string_t *msg);

  // sort the list of fields, trying to merge the duplicate
  // *res is set to false if something's wrong
  List::list_t<datatypefield_t> sort_xdatatype_fields(List::list_t<datatypefield_t,`H>, bool @res,
