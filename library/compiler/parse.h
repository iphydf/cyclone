...
using Core {
  namespace Parse {
    ...
    List::list_t<$(Absyn::type_t, List::list_t<BansheeIf::constraint_t>) @> parse_constraint_file(
        FILE @`H f);
  ...
  using Absyn {
    using List {
      namespace Parse {
        ...
        enum ConstraintOps { C_AND_OP, C_OR_OP, C_NOT_OP, C_EQ_OP, C_INCL_OP };
        ...
        extern struct Abstractdeclarator<`yy> { list_t<type_modifier_t<`yy>,`yy> tms; };
