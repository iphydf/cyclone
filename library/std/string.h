...
#ifndef CYCLONE_LIBRARY_STD_STRING_H
#define CYCLONE_LIBRARY_STD_STRING_H
__noinference__ {
#include "core.h"
...
#define bcopy(s, d, n) _bcopy((s), (d), _rounddiv(n, sizeof(*(s))), sizeof(*(s)))
  ...
  extern mstring_t<`r> rreplace_suffix(region_t<`r> r, buffer_t src, buffer_t curr_suffix,
                                       buffer_t new_suffix);
  ...
  extern List::list_t<intptr_t> explode(buffer_t s);
  extern List::list_t<intptr_t,`r> rexplode(region_t<`r>, buffer_t s);
  extern mstring_t implode(List::list_t<intptr_t> c);
}
#endif
