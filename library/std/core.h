...
#define true ((bool)1)
  ...
  namespace Core {
    ...
    mstring_t<`H> new_string(unsigned int x) @ensures(x <= numelts(return_value));
    /** [new_string(n)] allocates space for [n] characters on the heap
        and returns a pointer to the space.  All of the characters are
        set to NUL (0). */
    mstring_t<`r::E> rnew_string(region_t<`r>, unsigned int x) @ensures(x <= numelts(return_value));
...
#define umalloc(arg) qmalloc(Core::heap_region, Core::unique_qual, arg)
    ...
    void rufree(region_t<`r> h, `a::A\T *@aqual(UNIQUE) `r ptr
...
#define rcmalloc(arg) qmalloc(Core::heap_region, Core::refcnt_qual, arg)
  ...
  `a ?@autoreleased `r autorelease_handle(
           region_t<`r> h, `a::A\T ?`RC ptr) __attribute__((noliveunique(2)));
  ...
  struct NewDynamicRegion<`q::Q> {
    <`r> : single(`r) region_key_t<`r, `q> key;
  };
  ...
  struct NewDynamicRegion<UNIQUE> _new_ukey(unsigned int dis_reaps, const char *file,
                                            const char *func, int lineno);
  /** [new_ukey()] creates a fresh dynamic region [`r] and returns
      a unique key for that region. */

  struct NewDynamicRegion<REFCNT> _new_rckey(unsigned int dis_reaps, const char *file,
                                             const char *func, int lineno);
...
#define new_rckey() _new_rckey(1, __FILE__, "", __LINE__)
  ...
  `result open_region(region_key_t<`r, `q> key,
                      `arg arg,
                      `result body(region_t<`r> h, `arg arg); {}
                      : RESTRICTED >= `q);
  ...
  */
  ...
  `r __cyclone_internal_array_t<`a, `i, `r>;
  ...
  `a ?`r mkfat(__nn_cyclone_internal_array_t<`a, `i, `r::E> arr, sizeof_t<`a> s,
  ...
  struct ThinRes<`a::A, `r> {
    <`i::I> __cyclone_internal_array_t<`a, `i, `r> arr;
