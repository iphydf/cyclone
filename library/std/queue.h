...
namespace Queue {
  ...
  extern bool is_empty(queue_t<`a::B\T>);
  /** [is_empty(q)] returns true if [q] contains no elements, and
      returns false otherwise. */

  extern queue_t<`a::B\T> create();
  /** [create()] allocates a new, empty queue on the heap and returns it. */

  extern void add(queue_t<`a::B\T,`H>,`a x) __attribute__((consume(2)));
  /** [add(q,x)] adds [x] to the end of [q] (by side effect). */
  extern void radd(region_t<`r>, queue_t<`a::B\T,`r>,`a x
                   : RESTRICTED >= aquals(`a)) __attribute__((consume(3)));
  /** [radd(r,q,x)] is like [add(q,x)] except that the queue lives in
      the region with handle [r]. */

  extern void push(queue_t<`a::B\T,`H> q, `a x) __attribute__((consume(2)));
  /** [push(q,x)] adds [x] to the front of [q] (by side effect). */
  extern void rpush(region_t<`r> r, queue_t<`a::B\T,`r> q,`a x) __attribute__((consume(3)));
  ...
  extern int length(queue_t<`a::B\T>);
  ...
  extern `a *`U take_match(region_t<`r> r, queue_t<`a::A\T *`U,`r> q, bool(@f)(`b,`a *`U),
			 `b env);
  /** [take_match(r,q,f,c)] looks through the queue (starting from the
      front) and returns the element [x] for which [f(x,c)] returns
      true. */

  extern `a noalias_take(queue_t<`a::B\T> q, `a null_elem) __attribute__((consume(2)));
  /** [noalias_take(q)] is as take, above, but works when the queue
      ...
      the queue. */
  extern `a *`U ptr_take(queue_t<`a::A\T *`U> q);
