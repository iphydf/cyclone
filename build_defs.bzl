...
def _cyc_build(cc_rule, name, srcs = [], csrcs = [], boot_srcs = [], deps = [], cdeps = [], cycopts = [], linkopts = [], boot = False, **kwargs):
    if srcs:
        _cyc_compile(
            name = name + "_srcs",
            srcs = srcs,
...
def buildlib(name, src, hdrs, boot = False, visibility = None, deps = [], includes = []):
