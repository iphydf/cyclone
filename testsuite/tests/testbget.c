...
void *_reap_malloc(struct _ReapHandle *r, _AliasQualHandle_t aq, unsigned int s) {
  ...
  if (aq == CYC_CORE_REFCNT_AQUAL) {
    ...
    if (!result) throw(3) * ((int *)result) = 1;
    result += sizeof(int);
    return (void *)result;
  } else {
...
void do_free(struct Reap *r) {
  ...
  while (p) {
    if ((rand() % 10) < 4) {
      printf("%s: freeing pointer %d (0x%x) of size %d ... ", r->h.name, p->ix, p->ptr, p->size);
      fflush(stdout);
...
void do_alloc(struct Reap *r) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    fflush(stdout);
    if (check_contents(a, size, 0xff, 0)) exit(-1);
    printf("ok\n");
